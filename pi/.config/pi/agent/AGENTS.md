...
| `/u` | claude_4_5_haiku (GSA AI) | claude_4_6_sonnet (GSA AI) | claude_4_8_opus (GSA AI) |
...
- `/u` requires `GSAI_API_KEY` (or `USAI_API_KEY`) in `~/.config/zsh/local.zsh`. GSA AI models are served via https://api.gsa.usai.gov/api/v1. All `/u` tiers run at `thinking: off` — the ladder is purely by model capability. **Claude 4.8 Opus** (the newest USAi model) is available at the `pro` tier.
