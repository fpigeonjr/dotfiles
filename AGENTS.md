...
- **Pinned at v0.67.68**: pi v0.68.0 introduced a regression (issues #3487/#3488, fixed but not yet released on npm as of Apr 21 2026) — PR #3402 added custom Bedrock endpoint support by passing `model.baseUrl` to `BedrockRuntimeClient`, but all built-in pi Bedrock models have `baseUrl` hardcoded to `https://bedrock-runtime.us-east-1.amazonaws.com`. In v0.67.x that field was ignored; in v0.68.0 it overrides the SDK endpoint, so requests are sent to `us-east-1` while SigV4 credentials are scoped to `us-east-2`, causing `InvalidSignatureException: Credential should be scoped to a valid region`. To upgrade once a patch ships: `npm install -g @mariozechner/pi-coding-agent@latest`, test with `flexion-pi`, then update `lastChangelogVersion` in `pi/.config/pi/agent/settings.json` and remove this note.
