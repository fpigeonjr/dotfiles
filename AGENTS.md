...
- **Current version: v0.69.0**: Upgraded from v0.68.1 on 2026-04-22. Key additions: OSC 9;4 progress indicators in the tab bar during agent streaming/compaction (automatic, no config); stacked autocomplete providers via `ctx.ui.addAutocompleteProvider()` (no use case yet); terminating tool results via `terminate: true` (no custom tools yet). TypeBox 1.x migration and session-replacement ctx invalidation are breaking changes that do not affect `flexion-aws-status.ts`.
