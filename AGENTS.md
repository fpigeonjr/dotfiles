...
- **Current version: v0.70.2**: Upgraded from v0.69.0 on 2026-04-24. Key additions: OSC 9;4 tab-bar progress indicators now opt-in via `terminal.showTerminalProgress: true` (fixed to stay alive in Ghostty — enabled in settings.json); `retry.provider.timeoutMs` added for Bedrock long-running inference (set to 3600000 in settings.json); DeepSeek native provider (not applicable — on Bedrock); searchable `/login` provider selector; `retry.provider.maxRetryDelayMs` caps runaway provider retry delays. TypeBox 1.x migration and session-replacement ctx invalidation from 0.69.0 are breaking changes that do not affect `flexion-aws-status.ts`.
- **models.json — always set correct contextWindow/maxTokens**: Pi uses `contextWindow` from `models.json` to calculate the context fill % indicator. The file was originally created with `contextWindow: 32000` and `maxTokens: 4096` for all models — causing the indicator to hit 100% ~6× too early on Claude (200k window). Fixed 2026-04-25: Anthropic models → 200k/16384 (Opus: 32000 output), DeepSeek/Kimi-K2.5 → 128k/8192, Qwen3-Coder → 131072/8192. When adding new models, always look up the real values rather than accepting the defaults.
...
- **Bedrock model whitelist (Flexion endpoint)**: The `us.` short inference profile IDs are required (`us.anthropic.claude-sonnet-4-6` etc.) — bare model IDs fail with `ValidationException: Invocation of model ID with on-demand throughput isn't supported`. The following 6 models are pinned in `enabledModels` so Ctrl+P cycles among them: `us.anthropic.claude-sonnet-4-6` (default), `us.anthropic.claude-opus-4-7`, `us.anthropic.claude-haiku-4-5-20251001-v1:0`, `deepseek.v3.2` (strong coding benchmarks), `qwen.qwen3-coder-480b-a35b-v1:0` (480B MoE purpose-built for coding), `moonshotai.kimi-k2.5` (confirmed working on Flexion). `amazon.nova-premier-v1:0` was removed 2026-04-25 (pi dropped it from enabledModels during a session). All are the latest model from their respective providers on the Flexion endpoint. The endpoint also rejects `thinking.display` on the legacy `thinking: { type: "enabled", budget_tokens }` shape — error: `thinking.enabled.display: Extra inputs are not permitted`. Sonnet 4.6 and Opus 4.7 use the newer `adaptive` thinking shape (see `supportsAdaptiveThinking` in `pi-ai/dist/providers/amazon-bedrock.js`); Haiku 4.5 uses the legacy shape but the endpoint accepts `display` on that ID.
